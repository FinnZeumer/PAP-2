...
In der Auswertung werden die Spektren in Python geplottet.
...
Dies sind die Frauenhofer Linien, welche durch die Wechselwirkung von Luftmolekühlen und dem Licht entstehen. Eine nummerische Bestimmung der Frauenhofer Linien etwa via Python stellt sich aufgrund des Rauschen des Signals als herausfordernd heraus. 
...
Die Namen der bestimmten Frauenhofer Linien sind bereits in dem Plot gegeben. Die numerisch bestimmten Frauenhofer Linien, sowie die theoretischen Werte sind inklusive der \hyperref[eq:standardabweichung]{Standardabweichung (\ref*{eq:standardabweichung})} zwischen experimentellen und theoretischen Werten der \ctab{fraunhofer} zu entnehmen.

\begin{table}[h!]
    \caption{Vergleich der theoretischen und experimentellen Frauenhofer-Linien. Grün gekennzeichnet sind die Elemente der Balmer-Serie des Wasserstoffes.}
...
Dafür werden die \hyperref[eq:standardabweichung]{Standardabweichung (\ref*{eq:standardabweichung})} zwischen den jeweiligen Werten bestimmt. Für die $b$-Linien stellt sich eine Standardabweichung von $3.0\sigma$ ein, dies entspricht einer signifikanten Abweichung. Somit lässt sich statistisch sagen, dass die Werte verschieden sind. 
...
\paragraph{a) Weiße LEDs}
In diesem Plot sind die Spektren dreier LEDs dargestellt. Diese unterscheiden sich in ihrer\afz{Wärme}. Dabei beschreibt\afz{warmes Licht} ein rötlicheres Licht, und\afz{kaltes Licht} ein bläuliches Licht. Dabei ist die\afz{Basis} weißes Licht.
Genau dies ist auch dieser Abbildung zu entnehmen. In dem Wellenlängen bereich von $420-490 \mathrm{nm}$ befindet sich das sichtbare blaue Licht. Es zeigt sich, je wärmer das Licht ist, desto größer ist die Intensität in diesem Wellenlängen bereich im Verhältnis zur weißen Lichtintensität. 
Im Bereich von $500-700 \mathrm{nm}$ befindet sich dann das Spektrum, welches besagte weiße Basis darstellt.

\paragraph{b) Einfarbige LEDs}
...
\paragraph{d) Verschiedene Lichtquellen}
...
Die zweite Gruppe sind die breiten Spektren, welche für den Menschen als weiß wahrgenommen werden. Hierbei unterscheiden sich die Farbtemperaturen über den jeweiligen Blau- und Rotanteil der Spektren. Daher wird die Glühlampe als\afz{warm} wahrgenommen, denn ein großer Teil des Spektrums weißt eine große Intensität im roten Lichtbereich auf. Die LED dagegen wird als weißer bzw. kälter  wahrgenommen, da diese einen geringeren rotanteil hat. 
...
\img[0.85]{A2/Vgl_LQ_2c}{Überlagerung der untersuchten Lichtquellen nach verschiedenen Eigenschaften.}
...
\label{sec:A3}
In dieser Sektion wird die Strahlung der Natriumdampflampe untersucht. Insbesondere sollen die Emissionslinien bestimmt werden. Dafür wurde das Spektrum in verschiedene Bereiche aufgeteilt, die unterschiedliche Bereiche betonen und die Untersuchung erleichtern sollen. Die Auswertung erfolgt dabei numerisch via Pyhton. 
...
Es wird eine Ungenauigkeit von $\Delta \lambda = 1.2 \mathrm{nm}$ angenommen. Die \cabb{A3/Na_Spektren_Grid_3c} zeigt Auschnitte aus dem Natriumspektrum und die bestimmten Emissionslinien. 
Die bestimmten Wellenlängen stehen auf den Plots und sind der \ctab{Emissionslinien_Na} zu entnehmen. Die Benennung der Linien ist nach Plot (A-F) aufgeteilt und pro Plot werden die Linien von kleinen Wellenlängen zu den Großen durchnummeriert.
Bei dem Plot A ist den Experimentatoren ein Fehler aufgetreten, die linke Line (Line A1) ist in Sättigung. Dadurch ist der Wert größer bestimmt wurden.
...
Dafür muss jedoch $E_{3p}$ ermittelt werden. Dazu wird die Wellenlänge gesucht, welche dem Übergang von $3d$ nach $3p$ entspricht. Diese Eigenschaft erfüllt die Linie E4 mit $\lambda_\text{E4} = (819.2 \pm 1.2) \mathrm{nm}$. \ceq{neben1} wird nach $E_{3p}$ aufgelöst, und mit $m=3$ gelöst. Die Gleichung und die dazugehörige Ungenauigkeit nach \hyperref[eq:gauss_fehlfortpflanzung]{Gauß'scher Fehlerfortpflanzung (\ref*{eq:gauss_fehlfortpflanzung})} lauten
\eq{E_drei_pe}{
...
Berechnen der Wellenlängen liefert die Werte der \ctab{n1_lambdas}. Hier wurden auch die (am ehesten) dazu passenden Linien der experimentellen Messung herausgesucht und die \hyperref[eq:standardabweichung]{Standardabweichung (\ref*{eq:standardabweichung})} berechnet. Die maximale Abweichung der Werte wurde vom Experimentator auf 5 festgelegt.
...
Die zweite Nebenserie des Natriums wird analog zur Ersten bestimmt. Für diese Sektion werden die beiden Therme der \ceq{neben2} berücksichtigt. Daher muss hier die Energie des Grundzustandes

\eq{E_drei_s}{
    {E_\text{3s}} = E_\text{3p} - \frac{hc}{\lambda_D} \, , \qquad \Delta {E_\text{3s}} = \frac{hc}{{\lambda_D}^2} \Delta \lambda_D \, ,
}

berechnet werden. Die Ungenauigkeit wurde über die \hyperref[eq:gauss_fehlfortpflanzung]{Gauß'sche Fehlerfortpflanzung (\ref*{eq:gauss_fehlfortpflanzung})} bestimmt. Für die zweite Nebenserie wird die $D$-Line verwendet, um die Energie ${E_\text{3s}}$ zu berechnen. 
Die $D$-Linie entspricht dabei dem Wert C1 (Vgl. \ctab{Emissionslinien_Na}) und beschreibt den Übergang von $3p$ nach $3s$. Somit gilt $\lambda_\text{C1} = \lambda_D = (588.6 \pm 1.2) \mathrm{nm}$. Es wird erneut mit $m=3$ gerechnet, und final ergibt sich:
\res{E_\text{3s}}{-5.131}{0.005}{eV}

Zur Bestimmung der theoretsichen Wellenlängen der zweiten Natrium Nebenserie muss der Korrekturfaktor durch Umstellen der \ceq{rydberg} bestimmt werden. Somit ergibt sich für den Korrekturfaktor
\eq{korr_term_2n}{
    \delta_s = 3 - \sqrt{\frac{E_{Ry}}{{E_\text{3s}}}} \, , \qquad \Delta \delta_s = \frac{1}{2} \sqrt{\frac{E_{Ry}}{{E_\text{3s}}^3}} \, \Delta {E_\text{3s}} \, ,
}

wobei die Ungenauigkeit über die \hyperref[eq:gauss_fehlfortpflanzung]{Gauß'sche Fehlerfortpflanzung (\ref*{eq:gauss_fehlfortpflanzung})} bestimmt wurde. 
Für den Korrekturfaktor ergibt sich
\imp{\delta_s}{1.3717}{0.0008}{.}\label{eq:res_delta_s_1}

Für die \ceq{neben2} ergitb sich die Ungenauigkeit via \hyperref[eq:gauss_fehlfortpflanzung]{Gauß'scher Fehlerfortpflanzung (\ref*{eq:gauss_fehlfortpflanzung})} zu
\eq{lam_n2}{
    \Delta \lambda_{N2} = \frac{{\lambda_{N2}}^2}{hc} \sqrt{\left( \frac{2E_{Ry}}{(m - \delta_s)^3} \Delta \delta_s \right)^2 + (\Delta E_{3p})^2} \, .
}
Es werden die Linien für die Hauptquantenzahlen $m \in \{4:9\}$ berechnet. Die theoretisch erwarteten und die experimentell Bestimmten Wellenlängen der zweiten Nebenserie des Natriums sind der \ctab{n2_lambdas} zu entnehmen. Es wurde zudem die \hyperref[eq:standardabweichung]{Standardabweichung (\ref*{eq:standardabweichung})} zwischen den Werten ermittelt.
Es soll an der Stelle erwähnt sein, dass der Wert der Linie B5 bereits in der ersten Nebenserie genutzt wurde. Dies wird in der Diskussion aufgefasst werden. Beim Vergleich der Werte wurde darauf geachtet, dass keine Differenz größer als $7$ ist.
...
Zu letzt soll die theoretischen Wellenlängen der der Hauptserie nach \ceq{hauptserie} bestimmt werden. Zunächst muss erneut der Korrekturfaktor nach \ceq{rydberg} bestimmt werden. Es ergibt sich
\eq{hauptSerie_ushdfuisdfhf}{
    \delta_p = 3 - \sqrt{\frac{E_{Ry}}{E_{3p}}}\, , \qquad  \Delta \delta_p = \frac{1}{2} \sqrt{\frac{E_{Ry}}{{E_{3p}}^3}} \, \Delta E_{3p} \, .
}
Durch einsetzen der bekannten Parameter ergitbs sich der Korrekturfaktor von
\imp{\delta_p}{0.8793}{0.0008}{}

Es werden die Wellenlängen der Hauptserie für $m \in \{4,5\}$ bestimmt. Die Ungenauigkeit der \ceq{hauptserie} wird via \hyperref[eq:gauss_fehlfortpflanzung]{Gauß'scher Fehlerfortpflanzung (\ref*{eq:gauss_fehlfortpflanzung})} zu 
\eq{errHS}{
    \Delta \lambda_{H} = \frac{{\lambda_{H}}^2}{hc} \sqrt{\left( \frac{2 E_{Ry}}{(m - \delta_p)^3} \Delta \delta_p \right)^2 + (\Delta E_{3s})^2} \, ,
}
bestimmt. Die Ergebnisse sind er \ctab{H_lambdas} zu entnehmen.

\begin{table}[h!]
    \caption{Vergleich von theoretischen und experimentellen Wellenlängen der Haupt-Serie}
    \label{tab:H_lambdas}
    \centering
    \begin{tabular}{r | c c | l || c}
        \toprule
        m & $\lambda_{\text{H}}$ [nm] & $\lambda_{\text{exp}}$ [nm] & Idx & Std. Abw. $\sigma$ \\
        \midrule
...
\section{Serienenergie und Korrekturen der beiden Nebenserien des Natriums}
Diese Sektion untersucht erneut die erste und zweite Nebenserie des Natriumspektrums. Es sollen die charakteristischen Energien bestimmt werden. 

\subsection{Erste Nebenserie}
Zunächst werden die Wellenlängen der ersten Nebenserie nach \ctab{n1_lambdas} gegen die Quantenzahl $m$ geplottet. Dem Plot können sowohl die theoretischen Werte, als auch die experimentellen Werte entnommen werden. Durch die Experimentellen Werte wird eine Regression gezogen, welche über 
\eq{fitN1}{
    \lambda_{N1}(E_{Ry}, E_{3p}, \delta_d) = \frac{hc}{\frac{-E_{Ry}}{(m - \delta_d)^2} - E_{3p}}
}
beschrieben wird. Dabei wird der experimentelle Wert B5 hier nicht geplottet. Der Plot ist der \cabb{A4/N1_4a} zu entnehmen.
\img{A4/N1_4a}{Bestimmte Wellenlängen der ersten Nebenserie des Natriumatoms gegen die Quantenzahl $m$.}

Für die bestimmten Parameter der Regression ergibt sich
\eq{exp_n1_rey}{
    E_\text{Ry,1} = (-12.7 \pm 0.8) \mathrm{eV} \, ,
}
und 
\eq{exp_n1_E3p}{
    E_\text{3p,1} = (-3.012 \pm 0.014) \mathrm{eV} \, ,
}
sowie
\eq{exp_n1_delta}{
    \delta_\text{d,1} = (0.09 \pm 0.08)\, .
}

Die \hyperref[eq:standardabweichung]{Standardabweichung (\ref*{eq:standardabweichung})} von $E_\text{Ry,1}$ (\ceq{exp_n1_rey}) zum Literaturwert ergibt $1.15 \sigma$. 
Der Abweichung von $E_\text{3p,1}$(\ceq{exp_n1_E3p}) zum Wert $E_\text{3p}$ (\ceq{resE3p}) beträgt $0.9\sigma$.
Keine der beiden Abweichungen ist signifikant. Es zeigt sich zudem, dass der Einfluss des Korrekturfaktors $\delta_\text{d}$ keinen großen Einfluss gehabt hätte, die Werte würden nur geringfügig Abweichen.

Weiterführend soll die Güte der Regression bestimmt werden. Dazu muss die Summe
\eq{chi_sum}{
    \chi^2 = \sum_{m=3}^{12} \left(\frac{\lambda_{\text{Reg},m} - \lambda_{\text{Exp},m}}{\Delta \lambda_{\text{Exp},m}}\right)^2
} 
berechnet werden. $\lambda_{\text{Exp},m}$ beschreibt die experimentell bestimmte Wellenlänge und $\lambda_{\text{Reg},m}$ die Wellenlänge anhand der Regressionsfunktion in Abhänigkeit der Hauptquantenzahl $m$. 
Die Summe muss dann noch via
\eq{red_chi_sum}{
    \chi_\text{red}^2 = \frac{\chi^2}{f}
}
reduziert werden. Dabei beschreibt $f$ die Anzahl der Freiheitsgrade des Fits, welche über die Differenz der Anzahl der experimentellen Messpunkte und den tehoretisch berechneten Werten aus der Regressionsfunktion. 
Es ergibt sich
\eq{res_n1_chi_asd}{
    \chi_1^2 = 13.75 \quad \text{bzw.} \quad   \chi_{\text{red},1}^2 = 1.96 \, .
}

Dabei ist der Wert des reduzierten $\chi$-Quadrates weitaus kleiner als der optimale (Ziel-)Wert von 1.

Eine weitere Möglichkeit zur Beurteilung der Güte eines Fits ist die sogenannte Fit-Wahrscheinlichkeit. Sie gibt an, mit welcher Wahrscheinlichkeit bei einer Wiederholungsmessung eine größere $\chi^2$-Summe beobachtet wird, vorausgesetzt, dass der Fit die wahren Parameter korrekt beschreibt und die Messwerte normalverteilt um die Fitfunktion streuen.
Für die vorliegenden Daten ergibt sich bei $\chi^2 = 13.75$ und $f$ Freiheitsgraden eine Fit-Wahrscheinlichkeit von lediglich $5.58\%$. Dieser vergleichsweise kleine Wert deutet darauf hin, dass eine größere $\chi^2$-Summe nur selten zu erwarten ist. Dies spricht dafür, dass die Abweichungen zwischen Fit und Messwerten relativ groß sind.
    Im Einklang damit ist auch der reduzierte $\chi^2$-Wert von $\chi^2_{\mathrm{red}} = 1.96$, der deutlich über 1 liegt. Insgesamt weist dies darauf hin, dass entweder die Unsicherheiten unterschätzt wurden oder das verwendete Modell die Daten nicht vollständig bzw. richtig darstellt.

\subsection*{Zweite Nebenserie}
Analog zur ersten Nebenserie werden die Wellenlängen der zweiten Nebenserie gegen die Quantenzahl $m$ aufgetragen. Sowohl die theoretischen als auch die experimentell bestimmten Werte sind in \cabb{A4/N2_4b} dargestellt. Durch die experimentellen Daten wird erneut eine Regression gelegt, die durch die entsprechende Fitfunktion beschrieben wird.

\img{A4/N2_4b}{Bestimmte Wellenlängen der zweiten Nebenserie des Natriumatoms gegen die Quantenzahl $m$.}

Für die bestimmten Parameter der Regression ergibt sich
\eq{exp_n2_rey}{
    E_\text{Ry,2} = (-13 \pm 5) \mathrm{eV} \, ,
}
sowie
\eq{exp_n2_E3p}{
    E_\text{3p,2} = (-3.03 \pm 0.07) \mathrm{eV} \, ,
}
und
\eq{exp_n2_delta}{
    \delta_\text{s,2} = (1.5 \pm 0.5)\, .
}

Die \hyperref[eq:standardabweichung]{Standardabweichung (\ref*{eq:standardabweichung})} von $E_{\text{Ry},2}$ zum Literaturwert beträgt $0.24\sigma$ und für $E_{\text{3p},2}$ zum Wert aus der ersten Messreihe (\ceq{exp_n1_E3p}) beträgt diese $0.03\sigma$. Für den Vergleich von $E_{\text{3p},1}$ und $E_{\text{3p},2}$ ergibt sich eine Abweichung von $0.20\sigma$. Damit sind sämtliche Abweichungen als nicht signifikant zu bewerten.
Dies ist auf die (teilweise) großen Fehler zurückzuführen.

Für die vorliegenden Daten erhält man außerdem
\eq{res_n2_chi_asd}{
    \chi_2^2 = 6.26 \quad \text{bzw.} \quad \chi_{\text{red},2}^2 = 1.57 \, .
}

Der reduzierte $\chi^2$-Wert liegt damit über dem idealen Wert von 1, jedoch näher an diesem als im Fall der ersten Nebenserie, was auf eine insgesamt bessere Übereinstimmung zwischen Modell und Messwerten hindeutet.
Zusätzlich kann die Fit-Wahrscheinlichkeit zur Bewertung herangezogen werden. Diese gibt an, mit welcher Wahrscheinlichkeit bei einer Wiederholungsmessung eine größere $\chi^2$-Summe auftreten würde, unter der Annahme, dass das Modell korrekt ist und die Messwerte normalverteilt um die Fitfunktion streuen. Für die vorliegenden Daten ergibt sich eine Wahrscheinlichkeit von $18.03\%$. Dieser Wert ist deutlich größer als im Fall der ersten Nebenserie und spricht für eine insgesamt bessere Beschreibung der Daten durch das verwendete Modell, auch wenn noch gewisse Abweichungen vorhanden sind.
