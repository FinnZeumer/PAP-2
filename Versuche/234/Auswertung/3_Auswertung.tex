...
Es werden die Linien für Hauptquantenzahl $m \in \{4:9\}$ berechnet. Die theoretisch erwarteten und die experimentell BEstimmten Wellenlängen der zweiten Hauptserie des Natriums sind der \ctab{n2_lambdas} zu entnehmen. Es wurde zudem die \hyperref[eq:standardabweichung]{Standardabweichung (\ref*{eq:standardabweichung})} zwischen den Werten ermittelt.
...
berechnet werden. $\lambda_{\text{Exp},m}$ beschreibt die experimentell Bestimmte Wellenlänge und $\lambda_{\text{Reg},m}$ Wellenlänge anhand der Regressionsfunktion in Abhänigkeit der Hauptquantenzahl $m$. 
Die Summe muss dann noch via
\eq{red_chi_sum}{
    \chi_\text{red}^2 = \frac{\chi^2}{f}
}
reduziert werden. dabei beschreibt $f$ die Anzahl der Freiheitsgrade des Fits, welche über die Different der Anzahl der experimentelle Messpunkte und den tehoretisch berechneten Werten aus der Regressionsfunktion bestimmt. 
Es ergibt sich
\eq{res_n1_chi_asd}{
    \chi_1^2 = 13.75 \quad \text{bzw.} \quad   \chi_{\text{red},1}^2 = 1.96 \, .
}

Dabei ist der Wert des reduzierten $\chi$-Quadrates weitaus kleiner als der optimale (Ziel-)Wert.
...
Die \hyperref[eq:standardabweichung]{Standardabweichung (\ref*{eq:standardabweichung})} von $E_{\text{Ry},2}$ zum Literaturwert beträgt $0.24\sigma$, für $E_{\text{3p},2}$ zum Wert aus der ersten Messreihe $0.03\sigma$. Für den Vergleich von $E_{\text{3p},1}$ und $E_{\text{3p},2}$ ergibt sich eine Abweichung von $0.2\sigma$. Damit sind sämtliche Abweichungen als nicht signifikant zu bewerten.
Dies ist auf die Großen Fehler zurückzuführen.

Für die vorliegenden Daten erhält man
\eq{res_n2_chi_asd}{
