...
\paragraph{Dämpfung:}
Die Dämpfung wurde über die Steigung einer Regressionsfunktion bestimmt (\cabb{Damping_1}). Der Kreisel hat sich nutations- und präzessionsfrei in Rotationversetzt. Somit konnte die Dämpfung zu
...
Über die Dämpfung konnte weiterführend das erste Trägheitmoment bestimmt werden: $I_z$. Hierzu wurden vier verschiedene Massenkonfiguration untersucht, für alle wurde eine lineare Regression gelegt (\cabb{tp_gegen_omegaAVG3}).
...
bestimmt. 
Mit diesem Trägheitsmoment und der Drehfrequenzen $\Omega$ konnte zu letzt die Trägheitmomente $I_y = I_x$ bestimmt werden. Für $\Omega = 5.17\pm0.09$ Konnte das Trägheitmoment für $I_x$ bestimmt werden:
\res[-3]{I_x}{4.62}{0.12}{kg \, m^2}
Zuletzt kann das Trägheissmoment $I_x$ auch über die Nutationsbewegung bestimmt werden. Dazu wurde die Steigung $k$ aus der \cabb{omegaN_gegen_omegaF6b} bestimmt. Das Trägheissmoment wurde zu
...
Wie in \csec{A3} vermerkt, wurden zur Berechnung der Trägheitsmomente die gemessenen und nicht die theoretischen Endwerte verwendet. Die neuen Trägheitsmomente sind der \ctab{traegheitsmomente_theo} zu entnehmen.
...
\subsection{Verglecih zum theoretischen Trägheitsmoment}
Dem Praktikumsskript \cite{skriptPAP21} wurde der Radius die Masse der Kreiselgugel gegeben. Über diese Wete kann das theoretische Trägheitsmoment bestimmt werden. Die Formel für das Trägheitsmoment einer (Voll-)Kugel ist gegeben durch
\eq{tr_kugel_form}{
    I_K = \frac{2}{5} \, m \, r^2 \, ,
}
mit der Kugelmasse $m$ und dem Kugelradius $r$ \cite{wikiTraeg}. Einsetzen der Werte lieftert
\imp[-3]{I_K}{4.3}{0}{kg \, m^2}
für die Trägheissmomente $I_x = I_y = I_z$. Dies berücksichtigt die heruasstehende Stange nicht. 
...
\begin{table}[h!]
    ...
    \begin{tabular}{L L | L || L}
        \toprule
        \text{Trägheitsmoment } & I_\text{gem} [10^{-3} \mathrm{kg \, m^2}] & I_\text{theo} [10^{-3} \mathrm{kg \, m^2}] & \text{Std. Abw. } \sigma \\
        \midrule
...
Zuletzt sollen noch eine korrektur der beiden Graphen zur Bestimmung des Trägheissmomentes $I_x$ diskutiert werden (\cabb{Omega_gegen_omegaF5}, \cabb{omegaN_gegen_omegaF6b}). Deren Regressionsfunktionen sind nur an den Messwerten gefittet, und laufen beide nicht (exakt) durch den Nullpunkt. Beide Abbildungen haben den Achsenabschitt zur Ordinate vermerkt und sind als $\approx 0$ angenährt. 
Jedoch ist bekannt, dass beide Graphen erwartbar duch den Punkt $(0,0)$ laufen. Daher lassen sich die Plots auch durch diesen Punkt legen. Die neuen Plots sind der \cabb{Omega_gegen_omegaF5b} und \cabb{omegaN_gegen_omegaF6} zu entnehmen. Die Steigungen sind wie zu erwarten in derselben Größenordnung, jedoch leicht unterschiedlich. 

Für die erste Methode ergibt sich das Trägheissmoment 
\imp[-3]{I_{x,\text{null}}}{4.66}{0.19}{kg \, m^2}

und für die zweite
\imp[-3]{I_{x,\text{null}}}{4.74}{0.12}{kg \, m^2}
...
Die werte sind somit signifikan und liegen näher statistisch näher beieinander als die urprünnglichen $I_x$-Werte. Die Abweichugn der neuen Werte zu den ursprünglichen liegt für die erste Methode bei
...
\img{Omega_gegen_omegaF5b}{Korrigierter Plot durch den Nullpunkt zu Bestimmung $I_x$ nach erster Methode.}

\img{omegaN_gegen_omegaF6}{Korrigierter Plot durch den Nullpunkt zu Bestimmung $I_x$ über die Nutation.}
