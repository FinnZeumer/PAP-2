...
\begin{tabular}{llll}
\toprule
d [$\mathrm{m} \cdot 10^{-3}$] & r [$\mathrm{m} \cdot 10^{-3}$] & s [$\mathrm{m} \cdot 10^{-3}$] & $\overline{t} [\mathrm{s}]$ \\
\midrule
...
\res{k}{1961}{25}{\frac{1}{m \, s}}

Somit ist die Viskosität \(\eta\) über die umgestellte Stokes'sche Formel \ceq{eta_Fallrohr} berechnet werden, wobei die Unsicherheit \(\Delta \eta\) ebenfalls über die Gauß'sche Fehlerfortpflanzung bestimmt wird, wobei die Unsicherheiten der Steigung \(m\) und der Dichteunterschiede \(\rho_k - \rho_f\) berücksichtigt werden:
