...
Die vier Messreihen werden aufsummiert umd eine Gesamtmessreihe zu erhalten. Diese wird gegen die Zeit geplottet. Durch die Messdaten wird eine Regression der Form
\eq{reg_ag}{
   f(x) = A_1 e^{-\lambda_1 x} + A_2 e^{-\lambda_2 x} + y_0 \, . 
} 
gelegt. Dass die Regression einer Summation aus zwei Exponentialfunktionen entspricht liegt daran, dass zwei Isotope von Silber natürlich vorkommen. 
...
\begin{table}[h!]
    \centering
    \caption{Messergebnisse der Silbermessung.}
    \label{tab:AT1}
...
\begin{table}[h!]
    \centering
    \caption{Messergebnisse der Indiummessung.}
    \label{tab:AT2}
    \begin{tabular}{L | C | C | C | C}
        \toprule
        \text{Größe} &
...
\label{sec:A3}
Zuletzt sollen die bestimmten Regressionen auf ihre Güte geprüft werden. Dazu wird zunächst das Chi-Quadrat ($\chi^2$) bestimmt. Zudem wird dieses via Division mit den Freiheitsgraden dividert. Ist das $\chi_\text{red}^2 \approx 1$\footnote{Ein Wert von $=1$ wäre ein optimaler Fit.}, ist der Fit als sinnvoll zu erachten.
